...
describe('rules/rm-viewbox', () => {
	test('移除不必要的 ViewBox', async () => {
		...
		await rmViewBox(dom);
		expect(createXML(dom).replace(/>\s+</g, '><')).toBe('<svg x="0" y="0" width="100" height="100"><marker markerWidth="50" markerHeight="50"/><pattern/><symbol/><view viewBox="0 0 50 50"/></svg>');
	});
	...
	test('不一样的单位', async () => {
		...
		const dom = await parse(xml);
		await rmViewBox(dom);
		expect(createXML(dom).replace(/>\s+</g, '><')).toBe('<svg x="0" y="0" width="100pt" height="100pt" viewBox="0 0 100 100"/>');
	});
});
