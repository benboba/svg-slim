...
describe('path/compute-q', () => {
	it('absolute to relative', () => {
		stringifyPath(doCompute(parsePath('M 100 100z Q 110 110 110 120,210,220,210,220'))).should.equal('m100,100zq10,10,10,20,100,100,100,100');
	});

	it('relative to absolute', () => {
		stringifyPath(doCompute(parsePath('M100,100q-110,-50,-100,-80,-200,-200,-200,-200'))).should.equal('m100,100Q-10,50,0,20-200-180-200-180');
	});

	it('change to t', () => {
		stringifyPath(doCompute(parsePath('M0 0 Q0 100 100 100 Q 200 100 200 0 Z'))).should.equal('m0,0q0,100,100,100T200,0z');
		...
		M -50 -50 t 0,0,50,0,50,50 Q 50 0 50 100 100 100 150 100 150 50 150 0z`))).should.equal('m-50-50t0,0Q50,0,50,100q50,0,100,0,0-50,0-100zm0,0t0,0T0,0q50,0,50,100,50,0,100,0,0-50,0-100zm0,0t0,0,50,0T50,0q0,0,0,100,50,0,100,0,0-50,0-100z');
