...
describe('rules/combine-path', () => {
	it('合并路径', async () => {
		...
		await combinePath(dom, { option: { disregardFill: false, disregardOpacity: false } });
	...
	it('合并填充', async () => {
		...
		const dom = await parse(xml) as IDomNode;
		await combinePath(dom, { option: { disregardFill: true, disregardOpacity: false } });
	...
	it('合并半透明', async () => {
		...
		const dom = await parse(xml) as IDomNode;
		await combinePath(dom, { option: { disregardFill: true, disregardOpacity: true } });
