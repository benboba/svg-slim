...
describe('rules/shorten-decimal-digits', () => {
	it('rule false branch', async () => {
		...
		createXML(dom).replace(/>\s+</g, '><').should.equal('<svg><polygon stroke-width="1.999" opacity="0.00099999" points="200000 , 0.1 -1.1 0.5"/></svg>');
	});
	...
	it('badcase', async () => {
		...
		const dom = await parse(xml) as ITagNode;
		await combineStyle(dom);
		await shortenDecimalDigits([true, { sizeDigit: 2, angelDigit: 3 }], dom);
