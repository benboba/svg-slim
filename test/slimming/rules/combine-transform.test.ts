...
describe('rules/combine-transform', () => {
	...
	it('can"t apply transform', async () => {
		const xml = `<svg>
	...
	it('apply transform', async () => {
		...
		const config = createRuleConfig(mergeConfig(null), 'combine-transform');
		await combineTransform(dom, config);
		createXML(dom).replace(/>\s+</g, '><').should.equal('<svg><text dx="5" dy="6">1</text><text dx="8">2</text><rect ry="3" x="1"/><rect style="x:-60;y:100;width:50;height:100"/><rect width="2" height="1" rx="1" ry="2" x="-2"/><rect width="100" height="50" y="50"/><rect rx="2"/><rect width="6"/><rect rx="3" ry="6"/><rect x="11" y="5"/><rect rx="2" ry="1" x="5" y="5"/><line x1="1" x2="1"/><line y2="10"/><line x1="10"/><circle cy="1" cx="1"/><circle/><circle cx="10" cy="10"/><circle cx="6" r="10"/><ellipse cx="6" rx="10" ry="5"/><circle r="10" cx="5"/><ellipse cx="5" rx="10" ry="5"/><ellipse cy="1" rx="3" ry="5" cx="1"/><ellipse rx="5" ry="3"/><ellipse rx="3" ry="5" cy="20"/><ellipse rx="6" ry="10"/><circle r="4"/><ellipse rx="10" ry="4" cx="5"/><circle cx="5" r="4"/><circle r="3" cx="1"/></svg>');
	});

	it('apply transform poly and path', async () => {
		const xml = `<svg>
		...
		const config = createRuleConfig(mergeConfig(null), 'combine-transform');
		await combineTransform(dom, config);
		createXML(dom).replace(/>\s+</g, '><').should.equal('<svg><polyline points="1,2,2,3"/><polygon points="1,0,2,1"/><polyline points="0,0-1,1"/><polyline points="2,0,1-1"/><polygon points="0,0,2,2"/><polygon points="2,0,4,1"/><polygon transform="translate(1e5)" points="0,0,1,1,2,1,3,0,5,1,2,4"/><polygon transform="scale(985)" points="3,2,1,1,2,1,3,6,5,9,2,4"/><path d="m1,0h100v100a5,5,0,00-5-5zm5,5,3,3"/><path d="m101,101H-8V-8A5,5,0,006,6M0-98T99,9,6,6"/><path transform="translate(-1,1)" d="M1e9,1e9,1e9,0,0-1e9-1e9,0,0,1e9,1e9,0,0-1e9-1e9,0,0,1e9,1e9,0,0-1e9-1e9,0,0,1e9,1e9,0,0-1e9-1e9,0,0,1e9,1e9,0,0-1e9-1e9,0,0,1e9,1e9,0,0-1e9-1e9,0,0,1e9,1e9,0,0-1e9-1e9"/><path transform="scale(99)" d="M2e9,2e9,2e9,0,0-2e9-2e9,0,0,2e9,2e9,0,0-2e9-2e9,0,0,2e9,2e9,0,0-2e9-2e9,0,0,2e9,2e9,0,0-2e9-2e9,0,0,2e9,2e9,0,0-2e9-2e9,0,0,2e9,2e9,0,0-2e9-2e9,0,0,2e9,2e9,0,0-2e9-2e9"/><path d="m200,200H-8V-8A10,10,0,008,8M-2-98T98,8,10,10m0,0z"/><path d="m200,100H-8V-4A10,5,0,008,4M-2-49T98,4,10,5m0,0z"/><path transform="rotate(5)" d="m100,100h5,v5L-4,-4Q9,9,99,99q0,0,4,4l18,18z"/><path transform="rotate(30,15,15)" d="m100,100H-4V-4t0,0,4,4M-1,-49T49,4,5,5M0,0z"/><path transform="skewX(75)" d="m100,100h-40v-40T0,0,4,4M-1,-49T49,4,5,5m0,0z"/><path transform="skewY(-75)" d="m100,100H-4V-4t0,0,4,4M-1,-49T49,4,5,5m0,0z"/><path d="m183.3,199.4-143-71.66L5.77,12.3t0,0T12.6,19.5M-5.04-35.58t86.35,93.28T21.14,28.5m0,0z"/></svg>');
	});

	it('apply transform with animate', async () => {
		const xml = `<svg>
