...
describe('rules/compute-path', () => {
	it('重新计算路径', async () => {
		...
		createXML(dom).replace(/>\s+</g, '><').should.equal('<svg><path stroke="red" d="m100,100H10,20V10,20"/><path d="m0,0,100,200,200,99"/><path d="m5e5.1L-1-1,0,0h100v100H0z"/><path d="m80,80a45,45,0,0045,45,45,45,0,10-45-45z"/><path d="m0,0q0,100,100,100T200,0z"/><path d="m0,0c50,0,50,100,100,100s50-50,50-100z"/></svg>');
	});
	...
	it('道格拉斯普克', async () => {
		...
		await computePath(dom, config);
		createXML(dom).replace(/>\s+</g, '><').should.equal('<svg stroke="red"><path d="m0,0v300-1"/><path d="m0,0h300-1"/><path d="m300,0H100,300h-1"/><path d="m0,300V100,300v-1"/><path d="m0,0,20,20L10,10"/><path d="m300,300L0,0,20,20,10,10"/></svg>');
	});
	...
	it('曲线转直线', async () => {
		...
		await computePath(dom, config);
		createXML(dom).replace(/>\s+</g, '><').should.equal('<svg stroke="red"><path d="m0,0,2,3,5,4,6,2t1,1,9,9-1,2"/><path d="m1e3,0A4,4,1,0090,90m10,10Q0,0,9,9L8,7"/><path d="m100,100a6,6,1,002,3L35,35"/><path d="m0,0q100,0,100,100t5,5M0,0s100,0,100,100l6,6"/><path d="m90,90c5,5,5,0,8,8S5,5,6,6"/></svg>');
	});
	...
	it('with animate', async () => {
		...
		const config = createRuleConfig(mergeConfig(null), 'compute-path');
		await computePath(dom, config);
		createXML(dom).replace(/>\s+</g, '><').should.equal('<svg><path><animate attributeName="d" to="m5e5.1L-1-1,0,0h100v100H0z" values="m5e5.1L-1-1,0,0h100v100H0z"/></path></svg>');
	});
});
