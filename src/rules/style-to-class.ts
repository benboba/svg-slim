...
export const styleToClass = async (dom: IDom): Promise<void> => new Promise(resolve => {
	...
	for (const tag of tags) {
		...
		if (tagStyle) {
			...
			// 过短的样式没有必要抽取
			if (keys.length >= TRANSLATE_THRESHOLD) {
				const key = keys.join(';');
				if (!keyCache[key]) {
					keyCache[key] = {
						className: `c${(+new Date).toString(36)}`,
						tags: [tag],
						keys,
					};
				} else {
	...
	// 条件是至少命中 2 个
	if (Object.values(keyCache).some(cache => cache.tags.length > 1)) {
		...
		for (const [key, val] of Object.entries(keyCache)) {
