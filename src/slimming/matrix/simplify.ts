...
// 把 matrix 函数反转为简单函数
export function simplify(matrix: IMatrixFunc, digit1: number, digit2: number, digit3: number): IMatrixFunc {
    if (matrix.type === 'matrix' && !matrix.noEffect) {
        const mVal = matrix.val.join(',');
